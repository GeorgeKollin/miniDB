...
'''
    Φόρτωση της δυαδικής αναζήτησης και της 
    στοίβας εισαγωγών.
'''
from binarySearch import BinarySearch
from insertStack import InsertStack

class Database:
    '''
    Database class contains tables.
    
        Προσθήκη παραμέτρου 'insert_stack_size' στην 
        κατασκευαστική μέθοδο, για τον καθορισμό του 
        μεγέθους της στοίβας εισαγωγών. Μπορεί να αλλάξει 
        στη συνέχεια.
    '''
    def __init__(self, name, load=True, insert_stack_size=500):
        #   Καθορισμός μεγέθους στοίβας εισαγωγών
        self.insert_stack_size = insert_stack_size
        #   Αρχικοποίηση αντικειμένου στοίβας εισαγωγών
        self.in_stack = InsertStack()
        #   Αρχικοποίηση αντικειμένου δυαδικής αναζήτησης
        self.bs = BinarySearch(self)
        #   Μεταβλητή αποθήκευσης τρέχοντος πίνακα για 
        #   εισαγωγή εγγραφών
        self.current_table = None
        #   Αποδεκτοί μεταπίνακες. 
        #   Ο χρήστης δε θα μπορεί να ορίσει νέους μεταπίνακες
        self.acceptable_metatables = ['meta_length', 'meta_locks', 'meta_insert_stack', 'meta_indexes']
    ...
    '''
        Κατά τη μαζική εισαγωγή στοιχείων με χρήση στοίβας 
        εισαγωγών, η βάση δεν μπορεί να γνωρίζει, τουλάχιστον 
        προς το παρόν, πότε θα τελειώσουν οι εισαγωγές. 
        Συνεπώς, ο τελευταίος πίνακας, που συμπληρώνεται, 
        παραμένει κλειδωμένος, περιμένοντας για νέες εγγραφές. 
        Η παρακάτω μέθοδος, εν συναρτήσει με τη μεταβλητή 
        self.current_table ξεκλειδώνει και τον πίνακα που 
        εκκρεμεί, ούτως ώστε ο χρήστης να μπορεί να τον 
        χρησιμοποιήσει για άλλον σκοπό. 
        
        Η 'applyPendingActions' καλείται σε εντολές που 
        απαιτούν έλεγχο κλειδωμάτων. Έχει ενσωματωθεί στην 
        self.is_locked(<args>).
    '''
    def applyPendingActions(self):
        if self.current_table is not None:
            self.unlock_table(self.current_table)
            self._update()
            # self.save()
        return
    ...
    def save(self):
        ...
        for name, table in self.tables.items():
            with open(f'{self.savedir}/{name}.pkl', 'wb') as f:
                pickle.dump(table, f)
        #   Σε κάθε αποθήκευση της βάσης, η στοίβα εισαγωγών 
        #   αδειάζει
        self.in_stack.insert_stack = []
    ...
    '''
        Εδώ υπάρχει δυνατότητα χρήσης της οικουμενικής μεθόδου 
        dropDB(<database:Database object key>) που ορίζεται στο 
        αρχείο main.py, η οποία διαγράφει και τα στιγμιότυπα της 
        βάσης από τη RAM. Η dropDB, ωστόσο εκτελείται σφαιρικά 
        και όχι επί του αντικειμένου της βάσης.
    '''
    def drop_db(self):
    ...
    def create_table(self, name=None, column_names=None, column_types=None, primary_key=None, load=None):
        ...
        '''
        #   Προστασία από τον εσκεμμένο ορισμό νέων μεταπινάκων. 
        #   Για να είναι ένας μεταπίνακας αποδεκτός, θα πρέπει 
        #   να υπάρχει στη μεταβλητή self.acceptable_metatables
        if (name[:4]=='meta') and (name not in self.acceptable_metatables):
            return
    ...
    def insert(self, table_name, row, lock_load_save=True):
        ...
        if lock_load_save:
            ...
            self.lockX_table(table_name)
        '''
        #   Επανασχεδίαση της 'insert', με βάση τη 
        #   λειτουργικότητα της στοίβας εισαγωγών
        if lock_load_save:
            if self.current_table is None:
                self.current_table = table_name
                if self.is_locked(self.current_table, True):
                    return
                self.lockX_table(self.current_table)
            elif (self.current_table != table_name) or ((len(self.in_stack.insert_stack) + 1) > self.insert_stack_size):
        ...
        try:
            self.tables[table_name]._insert(row, self, insert_stack)
    ...
    def delete(self, table_name, condition):
        ...
        if self.is_locked(table_name):
            return
        self.lockX_table(table_name)
        deleted = self.tables[table_name]._delete_where(condition, self)
        self.unlock_table(table_name)
        self._update()
        # self.save()
    ...
    def sort(self, table_name, column_name, asc=False):
        ...
        if self.is_locked(table_name):
            return
        self.lockX_table(table_name)
        self.tables[table_name]._sort(column_name, asc=asc, database_object=self)
        self.unlock_table(table_name)
        self._update()
        # self.save()
    ...
    def is_locked(self, table_name, from_insert=False):
        '''
