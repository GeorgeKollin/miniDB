...
class Table:
    ...
    def _update(self):
        ...
        self.set_column_attribute()
        
    #   Δημιουργία ξεχωριστής μεθόδου απόδοσης χαρακτηριστικών 
    #   για μεμονωμένη κλήση της από την ενημερωμένη '_insert'
    ...
    def _insert(self, row, database_object=None, insert_stack=[]):
        '''
        Insert row to table
        '''
        ...
        #   Προσθήκη νέας εγγραφής στο τέλος του πίνακα
        for i in range(self._no_of_columns):
            self.columns[i].append(self.data[-1][i])
            
        #   Προσθήκη νέας εγγραφής στη στοίβα εισαγωγών
        if database_object is not None:
            database_object.in_stack.addToInsertStack(self._name, len(self.data) - 1)
    ...
    def _delete_where(self, condition, database_object=None):
        '''
        ...
        #   Διαγραφή αντίστοιχων εγγραφών από τη στοίβα εισαγωγών
        if database_object is not None:
            database_object.in_stack.updateInsertStack(table_name=self._name, referrer='delete_rows', indexes_to_be_processed=indexes_to_del)
    ...
    def _sort(self, column_name, asc=False, database_object=None):
        '''
        ...
        #   Ενημέρωση των δεικτών εγγραφών στη στοίβα εισαγωγών
        if database_object is not None:
            database_object.in_stack.updateInsertStack(table_name=self._name, referrer='sort_rows', indexes_to_be_processed=idx)
        self._update()
